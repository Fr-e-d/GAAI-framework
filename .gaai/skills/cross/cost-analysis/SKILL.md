...
inputs:
  - delivery_logs_dir: "Path to .gaai/.delivery-logs/ (default: auto-detected from project root)"
...
- `.gaai/.delivery-logs/` directory must exist with `*.log` JSONL files
...
Scan `.gaai/.delivery-logs/*.log` for available data. Count files, check for `result` events.
