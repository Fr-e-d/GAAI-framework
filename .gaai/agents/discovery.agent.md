...
- `domain-knowledge-research` — research current best practices and industry standards for a domain, producing GAAI-compatible AKUs with compatibility filtering. Activate when the Capability Readiness rule identifies a knowledge gap, or before designing skills in an unfamiliar domain. Broader than approach-evaluation (domain-wide, not decision-specific). Discovery reads the report and decides what to persist (`memory-ingest`), what informs skill creation, and what remains unresolved.
