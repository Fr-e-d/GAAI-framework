...
12. **MANDATORY — Register in backlog.** After writing all story files, add each story to `contexts/backlog/active.backlog.yaml` with:
   ...
   - **`impl_model` field — DO NOT SET unless overriding a default (DEC-93 + DEC-94)**:
     - **Default behavior (recommended)** : leave `impl_model` ABSENT in both story frontmatter and backlog entry. Per DEC-94 the daemon coerces routing automatically :
       - Tier 1 absent → secondary (cost-optimal, GLM)
       - Tier 2+ absent → primary (Sonnet, required for context window)
     - **Set `impl_model: secondary` ONLY on Tier 1 stories** where cost optimization is intentional and the GLM 200K context window is comfortably sufficient.
     - **NEVER set `impl_model: secondary` on Tier 2+ stories.** The daemon hard-gate at `daemon-dispatch.sh:766-779` will refuse the Impl phase dispatch with `TIER2_SECONDARY_REJECTED` and flip `phase_status: failed`. This is a structural failure, not a transient one — empirical evidence (E135S02 + E135S04 on 2026-05-06) shows Tier 2 cumulative input crosses the 167K compact threshold within ~16 events on the secondary route.
     - **Set `impl_model: primary` explicitly only on Tier 1 stories** that override the cost-optimal default for a sensitivity reason (security, compliance, accuracy-critical). For Tier 2+, leaving the field absent is preferred (DEC-94 coerces automatically — no redundant directive).
     - **Validation gate**: `validate-artefacts` (SKILL-VALIDATE-ARTEFACTS-001) returns BLOCKED on the `tier ≥ 2 + impl_model: secondary` combination. Stories that violate this rule must be either (a) decomposed to Tier 1 sub-stories per `PAT-STORY-SCOPE-DISCIPLINE-001`, (b) have the `impl_model: secondary` line removed (default coercion to primary), or (c) explicitly set `impl_model: primary`.
     - **Rationale**: a Discovery session on 2026-05-06 copied `impl_model: secondary` from an E136 pattern onto Tier 2 E137 stories without realizing DEC-94 had been added 02:44 the same morning. The daemon refused dispatch, marked `phase_status: failed`, leaving the stories stuck. Both the gate above (validation) and this skill text (prevention at authoring time) close the loop.
