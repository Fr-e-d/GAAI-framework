...
1. Read the Story template at `contexts/artefacts/stories/_template.story.md`. Read the parent Epic file. Derive story IDs using the parent Epic ID prefix (e.g., Epic E01 produces stories E01S01, E01S02, etc.).
   ...
   **CRITICAL — Custom Skill Contract (MUST execute for any story involving a custom skill):**
   ...
   - **Rationale:** Per DEC-126 §3, a custom skill may only enter autonomous Delivery via an
     explicit story `required_skills` contract — autonomy never selects a custom skill on its
     own. The bound output AC is the only QA-verifiable surface; without it,
     `validate-artefacts` returns BLOCKED (E178S04 AC2). This is the OSS enforcement twin of
     the cloud-side contract in E178S03. Both products enforce identical semantics per DEC-70.
