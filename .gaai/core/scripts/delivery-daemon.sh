...
reconcile_done_merged_worktrees() {
  ...
  while IFS= read -r sid; do
    ...
    if [[ "$wt_real" == "$proj_real" ]]; then
      log "[RECONCILE-SWEEP] $sid: skipped — wt_path resolved to PROJECT_DIR (safety guard)"
      continue
    fi
    ...
    if [[ -z "$is_merged" ]]; then
      log "[RECONCILE-SWEEP] $sid: skipped:unmerged (${branch_name} not in --merged origin/${effective_target})"
      continue
    fi
    ...
    if [[ $git_status_rc -ne 0 ]]; then
      log "[WARN][RECONCILE-SWEEP] $sid: git status failed in worktree (rc=$git_status_rc) — skipped:dirty (safe)"
      continue
    fi
    if [[ -n "$porcelain_out" ]]; then
      log "[WARN][RECONCILE-SWEEP] $sid: skipped:dirty — worktree has uncommitted/untracked content:"
      local _line_count=0
    ...
    if git -C "$PROJECT_DIR" worktree remove --force "$wt_path" 2>/dev/null; then
      log "[INFO][RECONCILE-SWEEP] $sid: removed (done+merged into ${effective_target}, clean)"
      ...
      log "[WARN][RECONCILE-SWEEP] $sid: remove failed (git lock contention?) — will retry next cycle"
    fi
