...
CANONICAL_TRUST_SURFACES='[".gaai/core/ci/premerge-policy.schema.json",".gaai/core/templates/ci/premerge-policy.json",".gaai/core/scripts/lib/premerge-proof.sh",".gaai/core/scripts/lib/local-admission-resolver.mjs",".gaai/core/scripts/lib/local-admission-executor.mjs",".gaai/core/scripts/lib/local-admission.sh"]'
...
[[ "$(jq -c '.covered_paths' "$TEMPLATE_PATH")" == "$CANONICAL_TRUST_SURFACES" ]] \
  && pass "T45a: covered_paths is exactly the six existing trust surfaces" \
