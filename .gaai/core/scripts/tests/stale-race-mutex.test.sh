...
# Set mtime to epoch 0 (always stale). Format literal computed at runtime
# via date to avoid OSS refcheck SHA-pattern false positive on contiguous digits.
if [[ "$(uname)" == "Darwin" ]]; then
  epoch_zero_fmt=$(date -u -r 0 +"%Y%m%d%H%M.%S")
  touch -m -t "$epoch_zero_fmt" "$LOCK_DIR/${T2_SID}.reconcile-in-progress"
else
