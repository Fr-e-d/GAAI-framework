...
T1_WS="aaaabbbb-cccc-dddd-eeee-ffffffff0001"
...
if echo "$T1_JSON" | python3 -c "import json,sys; d=json.load(sys.stdin); \
  ...
  assert s['headers']['X-GAAI-Workspace-Scope']=='aaaabbbb-cccc-dddd-eeee-ffffffff0001'; \
...
PROJECT_DIR="$T2_PROJECT_DIR"
export PROJECT_DIR
GAAI_WORKSPACE_ID="aaaabbbb-cccc-dddd-eeee-ffffffff0002"
export GAAI_WORKSPACE_ID
GUARD_FIRED_2=0
if [[ -f "${PROJECT_DIR}/.mcp.json" && -z "${GAAI_WORKSPACE_ID:-}" ]]; then
  GUARD_FIRED_2=1
...
T3_WS_A="11112222-aaaa-bbbb-cccc-ddddeeee0001"
T3_WS_B="99998888-ffff-eeee-dddd-ccccbbbb0002"
