...
# ═══════════════════════════════════════════════════════════════════════════════
# Phase-aware staleness: check_stale_in_progress dispatches recoverable phases
# ═══════════════════════════════════════════════════════════════════════════════
echo ""
echo "=== Phase-aware staleness: in-loop recovery dispatch ==="

STALE_BACKLOG_REL=".gaai/project/contexts/backlog/active.backlog.yaml"

# Shared awk extractor for check_stale_in_progress (used by all three scenario harnesses)
_stale_build_harness() {
  local harness_file="$1"
  local proj_dir="$2" lock_dir="$3" log_file="$4" threshold="$5"
  local relaunch_marker="$6"
  # Additional stub definitions passed as $7 (optional block inserted before eval)
  local extra_stubs="${7:-}"
  cat > "$harness_file" <<HARNESS
#!/usr/bin/env bash
set -uo pipefail
PROJECT_DIR="$proj_dir"
GAAI_PROJECT_DIR="$proj_dir/.gaai/project"
BACKLOG_REL="$STALE_BACKLOG_REL"
BACKLOG="\$PROJECT_DIR/\$BACKLOG_REL"
BACKLOG_FILE="\$BACKLOG"
LOCK_DIR="$lock_dir"
...
mkdir -p "\$LOG_DIR" "\$LOCK_DIR"
RED='' GREEN='' YELLOW='' BLUE='' CYAN='' BOLD='' NC=''
log() { echo -e "\$*" | sed "s/\\\033\[[0-9;]*m//g" >> "\$LOG_FILE"; }
...
_recovery_set_status() { return 0; }
_recovery_resolve_worktree() { echo "/nonexistent/\$1-workspace"; }
_emit_routing_record() { return 0; }
_recovery_relaunch() { touch "$relaunch_marker"; return 0; }
$extra_stubs
...
# ── Scenario A: qa_passed + retry_count < MAX → relaunch invoked, no fail ──
echo ""
echo "--- Scenario A: qa_passed + fresh counter → relaunch, no mark-failed ---"

STALE_A_STORY="TST-STALE-A"
STALE_A_DIR="$FIXTURE_DIR/stale-a-project"
STALE_A_LOCK_DIR="$FIXTURE_DIR/stale-a-locks"
STALE_A_LOG="$FIXTURE_DIR/stale-a-daemon.log"
mkdir -p "$STALE_A_LOCK_DIR"
touch "$STALE_A_LOG"

STALE_A_YAML="- id: $STALE_A_STORY
  status: in_progress
  phase_status: qa_passed
  delivery_pipeline: 3phase"

setup_git_repo "$STALE_A_DIR" "$STALE_A_YAML"
# Commit with subject matching "chore(${sid}): in_progress" for age calc.
git -C "$STALE_A_DIR" commit --allow-empty -m "chore($STALE_A_STORY): in_progress" -q
git -C "$STALE_A_DIR" push origin HEAD -q

STALE_A_HARNESS=$(mktemp /tmp/stale-a-harness-XXXXXX.sh)
STALE_A_RELAUNCH_MARKER="$STALE_A_LOCK_DIR/relaunch-called-${STALE_A_STORY}"
# STALENESS_THRESHOLD=-1 ensures (( age >= 0 > -1 )) is always true regardless of timing.
_stale_build_harness "$STALE_A_HARNESS" "$STALE_A_DIR" "$STALE_A_LOCK_DIR" "$STALE_A_LOG" "-1" "$STALE_A_RELAUNCH_MARKER"
chmod +x "$STALE_A_HARNESS"
bash "$STALE_A_HARNESS" 2>/dev/null
rm -f "$STALE_A_HARNESS"

echo "T18: _recovery_relaunch was called for qa_passed story"
if [[ -f "$STALE_A_RELAUNCH_MARKER" ]]; then
  pass "T18: relaunch marker found"
else
  fail "T18: relaunch marker missing — _recovery_relaunch was not invoked"
fi

echo "T19: retry counter file written for qa_passed story"
if [[ -f "$STALE_A_LOCK_DIR/.recovery-attempts-${STALE_A_STORY}" ]]; then
  counter_val=$(cat "$STALE_A_LOCK_DIR/.recovery-attempts-${STALE_A_STORY}" 2>/dev/null || echo "")
  if [[ "$counter_val" == "1" ]]; then
    pass "T19: counter incremented to 1"
  else
    fail "T19: counter value unexpected: '$counter_val' (expected 1)"
  fi
else
  fail "T19: retry counter file missing"
fi

echo "T20: no mark-failed git commit for qa_passed story (relaunch path taken)"
if git -C "$STALE_A_DIR" log "origin/main" --oneline 2>/dev/null | grep -q "failed.*${STALE_A_STORY}"; then
  fail "T20: unexpected mark-failed commit found for qa_passed story"
else
  pass "T20: no mark-failed commit (correct)"
fi

# ── Scenario B: qa_passed + retry_count == MAX → cap-exhaustion log, no relaunch ──
echo ""
echo "--- Scenario B: qa_passed + counter at cap → cap-exhaustion logged, no relaunch ---"

STALE_B_STORY="TST-STALE-B"
STALE_B_DIR="$FIXTURE_DIR/stale-b-project"
STALE_B_LOCK_DIR="$FIXTURE_DIR/stale-b-locks"
STALE_B_LOG="$FIXTURE_DIR/stale-b-daemon.log"
mkdir -p "$STALE_B_LOCK_DIR"
touch "$STALE_B_LOG"

STALE_B_YAML="- id: $STALE_B_STORY
  status: in_progress
  phase_status: qa_passed
  delivery_pipeline: 3phase"

setup_git_repo "$STALE_B_DIR" "$STALE_B_YAML"
git -C "$STALE_B_DIR" commit --allow-empty -m "chore($STALE_B_STORY): in_progress" -q
git -C "$STALE_B_DIR" push origin HEAD -q

# Pre-create counter at cap (3 = GAAI_INLOOP_RECOVERY_MAX default).
printf '3\n' > "$STALE_B_LOCK_DIR/.recovery-attempts-${STALE_B_STORY}"

STALE_B_HARNESS=$(mktemp /tmp/stale-b-harness-XXXXXX.sh)
STALE_B_RELAUNCH_MARKER="$STALE_B_LOCK_DIR/relaunch-called-${STALE_B_STORY}"
_stale_build_harness "$STALE_B_HARNESS" "$STALE_B_DIR" "$STALE_B_LOCK_DIR" "$STALE_B_LOG" "-1" "$STALE_B_RELAUNCH_MARKER"
chmod +x "$STALE_B_HARNESS"
bash "$STALE_B_HARNESS" 2>/dev/null
rm -f "$STALE_B_HARNESS"

echo "T21: _recovery_relaunch NOT called when counter at cap"
if [[ -f "$STALE_B_RELAUNCH_MARKER" ]]; then
  fail "T21: relaunch marker found despite counter at cap — should not have relaunched"
else
  pass "T21: no relaunch (cap exhausted correctly)"
fi

echo "T22: cap-exhaustion log message written"
if grep -q "recovery cap reached" "$STALE_B_LOG" 2>/dev/null; then
  pass "T22: cap-exhaustion log message found"
else
  fail "T22: cap-exhaustion message missing — got: $(cat "$STALE_B_LOG" 2>/dev/null | head -5)"
fi

echo "T23: recovery-attempts counter file deleted on cap exhaust"
if [[ -f "$STALE_B_LOCK_DIR/.recovery-attempts-${STALE_B_STORY}" ]]; then
  fail "T23: counter file still present — should have been deleted on cap exhaust"
else
  pass "T23: counter file deleted on cap exhaust"
fi

# ── Scenario C: not_started → existing mark-failed behavior preserved (regression guard) ──
echo ""
echo "--- Scenario C: not_started → stale detection triggered, no relaunch (regression guard) ---"

STALE_C_STORY="TST-STALE-C"
STALE_C_DIR="$FIXTURE_DIR/stale-c-project"
STALE_C_LOCK_DIR="$FIXTURE_DIR/stale-c-locks"
STALE_C_LOG="$FIXTURE_DIR/stale-c-daemon.log"
mkdir -p "$STALE_C_LOCK_DIR"
touch "$STALE_C_LOG"

STALE_C_YAML="- id: $STALE_C_STORY
  status: in_progress
  phase_status: not_started
  delivery_pipeline: 3phase"

setup_git_repo "$STALE_C_DIR" "$STALE_C_YAML"
git -C "$STALE_C_DIR" commit --allow-empty -m "chore($STALE_C_STORY): in_progress" -q
git -C "$STALE_C_DIR" push origin HEAD -q

STALE_C_HARNESS=$(mktemp /tmp/stale-c-harness-XXXXXX.sh)
STALE_C_RELAUNCH_MARKER="$STALE_C_LOCK_DIR/relaunch-called-${STALE_C_STORY}"
_stale_build_harness "$STALE_C_HARNESS" "$STALE_C_DIR" "$STALE_C_LOCK_DIR" "$STALE_C_LOG" "-1" "$STALE_C_RELAUNCH_MARKER"
chmod +x "$STALE_C_HARNESS"
bash "$STALE_C_HARNESS" 2>/dev/null
rm -f "$STALE_C_HARNESS"

echo "T24: _recovery_relaunch NOT called for not_started story"
if [[ -f "$STALE_C_RELAUNCH_MARKER" ]]; then
  fail "T24: relaunch was unexpectedly called for not_started story"
else
  pass "T24: no relaunch for not_started (correct)"
fi

echo "T25: stale detection triggered for not_started story (mark-failed path entered)"
if grep -q "STALE:.*${STALE_C_STORY}" "$STALE_C_LOG" 2>/dev/null; then
  pass "T25: STALE detection log found — existing mark-failed path entered"
else
  fail "T25: STALE detection log missing — regression: not_started stale check did not fire"
fi
