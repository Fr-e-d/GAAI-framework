...
# ── Section 7: Implementation delegation (orchestrator pattern) ──────────
# When the plan is large enough to risk monolithic context exhaustion, the
# lead impl agent delegates each plan step to a Task sub-agent. Each
# sub-agent runs in an isolated context (no parent transcript), does the
# scoped work, returns a 1-2K token summary. Lead aggregates summaries
# into impl-report.md and runs a final integration test pass.
#
# Pattern alignment :
#   - Anthropic harness recommendation : "Multiple specialized agents
#     handle focused tasks with clean context windows. The lead agent
#     coordinates high-level strategy while subagents perform deep work,
#     returning condensed summaries (1,000-2,000 tokens)."
#   - In-process delegation via Task tool — same parent auth + base URL +
#     model (homogeneous : Sonnet→Sonnet sub-agents on primary path,
#     GLM→GLM sub-agents on secondary path).
#   - Cognitive heavy-lifting (architectural decomposition, dep mapping)
#     is done UPSTREAM in the Plan phase. The lead orchestrator is
#     mechanical : read plan → dispatch each step → aggregate.
#
# Trigger : the lead decides via plan analysis (≥4 implementation steps OR
...
1. Compose your dispatch plan in the notes file. List each step + its
   file scope + which steps can run independently vs sequentially based
   on file overlap declared in the plan.

2. For each step (or grouped steps with no file overlap), invoke the
   Task tool :

     Task(
       subagent_type='general-purpose',
       description='<3-5 word label, e.g. "AC2 webhook handler">',
       prompt='<see template below>'
     )

   Sub-agent prompt template :
   """
   You are an implementation sub-agent for story <STORY_ID>, executing
   this specific plan step :

   <PASTE PLAN STEP VERBATIM, INCLUDING FILE PATHS + LINE RANGES>

   Acceptance criteria addressed by this step : <AC list>

   Your tasks, in order :
     1. Read scoped files only, ≤200 lines per Read (use offset/limit).
     2. Make the edits per the plan step.
     3. Run targeted tests for the modified files only
        (e.g. \`pnpm test -- src/path/to/file.test.ts 2>&1 | tail -100\`).
     4. Return a structured summary (≤1500 tokens) :
        ## Files Modified — bullets, one-line rationale per file
        ## Tests Run — commands + pass/fail counts
        ## Open Concerns — anything cross-AC or that needs lead attention
        ## Status — DONE | PARTIAL | BLOCKED

   CONSTRAINTS :
     - DO NOT write impl-report.md (the lead does that).
     - DO NOT git commit or git add (the lead does that at end).
     - DO NOT modify files outside your declared scope.
     - Stay within \${GAAI_WORKTREE_PATH}.
     - Use Write/Edit, never Bash heredoc, for any persistence.
   """

3. After all sub-agents complete :
   a. Append each summary to your notes file under "## Sub-agent reports"
   b. Identify any BLOCKED / PARTIAL — address yourself or spawn a
      follow-up sub-agent
   c. Run a FULL test pass yourself (single Bash) :
      \`pnpm test 2>&1 | tail -200\`  (or the equivalent for this repo)
   d. Aggregate sub-agent summaries into impl-report.md per the
      MANDATORY HANDOFF section structure above
   e. Single \`git add -A\` + \`git commit\` at the end

WHY this pattern : your context stays small (~50K — plan + summaries).
Each sub-agent runs in a clean isolated context (~50-80K each, scoped
files only). No cumulative tool-output accumulation on the lead. This
prevents the rapid_refill_breaker observed empirically on Tier 2
cross-cutting stories when the lead agent accumulated raw outputs
across 10+ files.

=== END IMPLEMENTATION DELEGATION ===
