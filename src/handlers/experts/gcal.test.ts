...
// Helper: create mock Supabase single-row response (PostgREST returns array, .single() picks first)
function postgrestSingle(data: object) {
...
describe('handleGcalAuthUrl', () => {
  it('returns 200 with correct auth URL params', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(postgrestSingle({ id: expertId }));
...
describe('handleGcalCallback', () => {
  it('stores encrypted tokens and redirects to gcal-connected on success', async () => {
    // Mock 1: expert lookup
    vi.mocked(fetch).mockResolvedValueOnce(postgrestSingle({ id: expertId }));
    // Mock 2: Google token exchange
    vi.mocked(fetch).mockResolvedValueOnce(new Response(
      JSON.stringify({ access_token: 'at_plain', refresh_token: 'rt_plain', expiry_date: Date.now() + 3600000 }),
      { status: 200 }
    ));
    // Mock 3: Google userinfo
    vi.mocked(fetch).mockResolvedValueOnce(new Response(
      JSON.stringify({ email: 'expert@gmail.com' }),
      { status: 200 }
    ));
    // Mock 4: Supabase update
    vi.mocked(fetch).mockResolvedValueOnce(postgrestUpdate());
  ...
  it('does NOT include gcal_refresh_token in update when absent from token exchange (AC3)', async () => {
    // Track what update payload was sent
    let updateBody = '';

    // Mock 1: expert lookup
    vi.mocked(fetch).mockResolvedValueOnce(postgrestSingle({ id: expertId }));
    // Mock 2: token exchange WITHOUT refresh_token
    vi.mocked(fetch).mockResolvedValueOnce(new Response(
    ...
    // Mock 4: Supabase update — capture request body
    vi.mocked(fetch).mockImplementationOnce(async (input, init) => {
      updateBody = typeof init?.body === 'string' ? init.body : JSON.stringify(init?.body ?? '');
      return postgrestUpdate();
    });
    ...
    // The update payload should NOT contain gcal_refresh_token
    expect(updateBody).not.toContain('gcal_refresh_token');
  });
